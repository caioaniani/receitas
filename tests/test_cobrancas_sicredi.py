...
# ── pedido de baixa de título registrado (instrução 02, 12/07/2026) ─────────

def _registrada(admin_user):
    """Cobrança levada até 'registrada' (remessa + retorno de registro)."""
    from app.services.sicredi_cnab import gerar_remessa, processar_retorno
    cob = _cobranca()
    gerar_remessa([cob], user_id=admin_user.id)
    processar_retorno(_linha_retorno_tipo1(cob.nosso_numero, '02') + '\r\n')
    db.session.refresh(cob)
    assert cob.status == 'registrada'
    return cob


def test_remessa_baixa_instrucao_02_e_status(app, admin_user):
    """gerar_remessa_baixa: detalhe com instrução 02 nas posições 109-110,
    mesmo nosso número do cadastro, status → baixa_solicitada."""
    from app.services.sicredi_cnab import gerar_remessa_baixa
    with app.app_context():
        cob = _registrada(admin_user)
        rem, erros = gerar_remessa_baixa([cob], user_id=admin_user.id)
        assert erros == []
        db.session.refresh(cob)
        assert cob.status == 'baixa_solicitada'
        detalhe = rem.conteudo.split('\r\n')[1]
        assert detalhe[108:110] == '02'          # instrução: pedido de baixa
        assert detalhe[47:56] == cob.nosso_numero
        assert len(detalhe) == 400


def test_remessa_baixa_recusa_titulo_nao_registrado(app, admin_user):
    """Só título REGISTRADO pede baixa — pendente/remessa são recusados e
    nada é gravado."""
    from app.services.sicredi_cnab import gerar_remessa_baixa
    with app.app_context():
        cob = _cobranca()                         # pendente
        rem, erros = gerar_remessa_baixa([cob], user_id=admin_user.id)
        assert rem is None and erros
        db.session.refresh(cob)
        assert cob.status == 'pendente'


def test_retorno_confirma_baixa_solicitada(app, admin_user):
    """Ocorrência 10 (baixado conforme instruções) fecha o ciclo:
    baixa_solicitada → baixada."""
    from app.services.sicredi_cnab import gerar_remessa_baixa, processar_retorno
    with app.app_context():
        cob = _registrada(admin_user)
        gerar_remessa_baixa([cob], user_id=admin_user.id)
        res = processar_retorno(
            _linha_retorno_tipo1(cob.nosso_numero, '10') + '\r\n')
        assert res['baixadas'] == 1
        db.session.refresh(cob)
        assert cob.status == 'baixada'


def test_retorno_baixa_rejeitada_volta_pra_registrada(app, admin_user):
    """Ocorrência 27 (baixa rejeitada): o título segue VIVO no banco — a
    cobrança volta pra 'registrada' (nunca 'rejeitada', que é entrada
    recusada) e o motivo fica visível."""
    from app.services.sicredi_cnab import gerar_remessa_baixa, processar_retorno
    with app.app_context():
        cob = _registrada(admin_user)
        gerar_remessa_baixa([cob], user_id=admin_user.id)
        linha = list(_linha_retorno_tipo1(cob.nosso_numero, '27'))
        linha[318:328] = list('15        ')
        res = processar_retorno(''.join(linha))
        db.session.refresh(cob)
        assert cob.status == 'registrada'
        assert 'baixa rejeitada' in cob.motivo_retorno
        assert any('BAIXA REJEITADA' in d for d in res['detalhes'])


def test_rota_solicitar_baixa(app, admin_user):
    """POST /cobrancas/<id>/solicitar-baixa gera a remessa e a tela mostra
    o botão só em título registrado."""
    with app.app_context():
        cob = _registrada(admin_user)
        c = app.test_client()
        _login(c, admin_user.id)
        corpo = c.get('/cobrancas/').get_data(as_text=True)
        assert 'pedir baixa' in corpo
        r = c.post(f'/cobrancas/{cob.id}/solicitar-baixa',
                   follow_redirects=True)
        assert r.status_code == 200
        assert 'PEDIDO DE BAIXA' in r.get_data(as_text=True)
        db.session.refresh(cob)
        assert cob.status == 'baixa_solicitada'
