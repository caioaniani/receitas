...
def test_regras_antes_do_preparo_e_dos_produtos(app, admin_user, cliente):
    """Ordem default 21/07 (dono: "o rodapé venha para cima" — SUBSTITUI o
    "produtos para cima" de 20/07): regras → métodos → produtos. A posição
    de cada seção é arrastável em Editar regras."""
    from app.models import AppConfig
    _receita('Brioche')
    ...
    assert body.index('Métodos de preparo') < body.index('Brioche')
