...
def _api_vendas_impl():
    ...
    if not ao_vivo:
        ...
        resp = jsonify(
            ...
            sem_itens_total=d['sem_itens_total'],
            sem_itens_n=d['sem_itens_n'],
            delivery_sem_itens_total=d['delivery_sem_itens_total'],
            delivery_sem_itens_n=d['delivery_sem_itens_n'],
            por_pagamento=d['por_pagamento'],
            por_canal=d['por_canal'],
            por_loja=d['por_loja'],
            por_loja_sem_itens=d['por_loja_sem_itens'],
            por_loja_sem_itens_n=d['por_loja_sem_itens_n'],
    ...
    try:
        resp = jsonify(
            ...
            total_valor=total,
            sem_itens_total=sem_itens_total,
            sem_itens_n=sem_itens_n,
            delivery_sem_itens_total=delivery_sem_itens_total,
            delivery_sem_itens_n=delivery_sem_itens_n,
...
def tiny_pdv():
    ...
    return render_template(
        ...
        sugestoes=sugestoes,
        # Quantas sugestoes sao 100% (aceitaveis em lote) — so informa o
        # rotulo do botao; o servidor recomputa no POST.
        n_lote=sum(1 for s in sugestoes.values() if s.get('lote')),
