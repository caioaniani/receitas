...
def _extrair_checkout_url(body):
    """Extrai a URL do checkout hospedado da resposta do Order. O Pagar.me
    v5 devolve em `checkouts[0].payment_url` (lista, como charges). Defensivo
    contra variacoes de chave."""
    checkouts = body.get('checkouts') or []
    if checkouts and isinstance(checkouts, list):
        c0 = checkouts[0] or {}
        return c0.get('payment_url') or c0.get('url')
    # fallback: alguns retornos colocam direto
    return body.get('payment_url') or body.get('url')


def criar_link_pagamento(pedido, *, success_url=None, expira_em_min=60):
    """Cria um Order com checkout HOSPEDADO pelo Pagar.me (link de pagamento).
    A pagina hospedada aceita Apple Pay (Safari/iOS), Pix e cartao — sem a
    gente implementar carteira nenhuma. Devolve:
      {ok, order_id, payment_url, erro?}

    O webhook 'order.paid' cai no nosso sistema igual ao Pix/cartao (casa
    por `code`/order_id) — entao a baixa de estoque e o e-mail seguem o
    mesmo caminho. Best-effort: nunca levanta excecao.

    ⚠️ VALIDAR NO SANDBOX antes de producao: o objeto `checkout` da v5 tem
    campos especificos (accepted_payment_methods, installments, expires_in)
    que a doc do Pagar.me detalha; se o sandbox recusar algum campo, ajustar
    aqui. A estrutura geral (Order + payment_method='checkout') esta certa.
    """
    total = _centavos(pedido.valor_total)
    checkout = {
        'expires_in': int(expira_em_min) * 60,
        'default_payment_method': 'credit_card',
        # Apple Pay aparece DENTRO do fluxo de cartao na pagina hospedada
        # (Safari/iOS) — nao precisa de entrada propria aqui.
        'accepted_payment_methods': ['credit_card', 'pix'],
        'customer_editable': False,
        'skip_checkout_success_page': False,
        'credit_card': {
            'statement_descriptor': 'O PAO PADARIA',
            # 1x a vista (B2C). Trocar pra varias parcelas se quiser depois.
            'installments': [{'number': 1, 'total': total}],
        },
        'pix': {'expires_in': int(expira_em_min) * 60},
    }
    if success_url:
    ...
    if status not in (200, 201):
        erro = body.get('message') or body.get('_erro') or f'HTTP {status}'
        logger.warning('pagarme criar_link_pagamento HTTP %s: %s', status, body)
        return {'ok': False, 'erro': erro, 'http': status}
    url = _extrair_checkout_url(body)
    if not url:
        logger.warning('pagarme link sem payment_url | body=%s', body)
        return {'ok': False, 'erro': 'Pagar.me nao retornou o link de pagamento.',
                'order_id': body.get('id')}
    return {
        'ok': True,
        'order_id': body.get('id'),
