...
- registrar_desperdicio_lote: REGRA CRITICA — quando o usuario passar uma LISTA de itens vencidos/descartados ('anota essas sobras', '2 croissants vencidos, 3 pao frances, 1 nutella...'), CHAME ESTA TOOL UMA VEZ SO com todos os itens no array `itens`. NUNCA chame registrar_desperdicio multiplas vezes — o sistema ignora chamadas em paralelo. Motivo unico pro lote (default vencido). Mesma regra de loja: SEMPRE preencha loja_nome; se nao tiver, pergunte.
- criar_venda_b2b: venda da INDUSTRIA pra cliente externo (hotel, restaurante). Baixa do FREEZER (EstoqueProducao), NAO de loja. Diferente de criar_pedido (pedido_de_loja vai pra entrega futura nas lojas proprias). Use quando o usuario disser 'vendi pro hotel X', 'fatura essa venda pro restaurante Y'. Cliente_nome: passe como o usuario disse — servidor faz fuzzy match. Se nao achar cliente cadastrado, fica como avulso (ok). Preços: NAO preencha preco_unitario a nao ser que o usuario explicite — servidor pega do cadastro (Receita.preco_venda / Produto.preco_atacado) + desconto do cliente. Parcelas: omita se for a vista, ou liste {{vencimento, valor, forma_pagamento}}.
