...
def _e_handoff_preguicoso_em_compra(historico, resultado_bot, conv_id=None):
    ...
    # SEM `motivo=` de proposito (revisao 19/09/2026): o motivo e texto que o
    # PROPRIO modelo escreveu na tool — o detector deterministico de "venda
    # em risco" nao pode ser calado por quem ele vigia. O pedido explicito
    # de humano entra aqui so pela FALA DO CLIENTE (`_quer_humano` abaixo).
...
def avaliar_abandono(historico, *, conv_id=None, nome_contato='', minutos_sem_resposta=0):
    ...
    # Mesma fonte unica do `avaliar` (`texto_da_mensagem`, importada acima):
    # turno so-imagem registra '[imagem enviada]' em vez do texto de um
    # turno anterior.
    ultima_msg = ''
    for m in reversed(historico or []):
        if m.get('role') == 'user' and texto_da_mensagem(m):
